...
protected:
  ...
  /// Adds all the constraints eagerly by implementing Algorithms 7.4.1 and
  /// 7.4.2 of Kroening and Strichman (which describe how to instantiate Lemmas
  /// 1 and 2 of Christ and Hoenicke).
  void add_array_constraints();
  void add_array_Ackermann_constraints();
  /// Extend the weak equivalence graph with an array term \p a, the graph node
  /// of which is \p i (as computed via \ref arrays).
  void collect_arrays(const exprt &a, wegt::node_indext i);
  /// Invoke \ref record_array_index for any index expressions contained within
  /// array terms.
  void collect_indices(const exprt &);

  // Map array terms to node indices in the weak equivalence graph.
  typedef numberingt<exprt, irep_hash> array_numberingt;
  ...
  typedef std::map<wegt::node_indext, index_sett> index_mapt;
